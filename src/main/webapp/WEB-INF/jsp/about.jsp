...
<html>
		...
		<div class="mui-content">
			<div class="mui-content-padded">
 			<c:forEach var="menu" items="${secondMenuList }">
				<div class="mui-btn <c:if test="${secondMenuId==menu.id}"> mui-btn-primary</c:if>" onclick="jumpAbout('${menu.id}')">
				${menu.name }
				</div>
			</c:forEach>
			</div>
	...
	jQuery(document).ready(function() {
		...
		mui.init({
			...
			function jumpAbout(secondMenuId){
				window.location.href="${contextpath}/about.htm?secondMenuId="+secondMenuId;
			}
			function jumpDetail(id){
				window.location.href='${contextpath}/aboutDetail.htm?id='+id;
			}
		</script>
