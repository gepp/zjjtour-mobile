			...
			<c:forEach var="item" items="${quanjingList }">
				...
				<c:if test="${secondMenuId!=1086 }">
					<li class="mui-table-view-cell mui-media"
						onclick="jumpDetail('${item.id}')"><a href="javascript:;">
							<img class="mui-media-object mui-pull-left"
							src="${item.smallimg } ">
							<div class="mui-media-body">
								${item.title }
								<p class="mui-ellipsis-2">${item.abstract_content }</p>
							</div>
					</a></li>
				</c:if>
			</c:forEach>
...
<script>
	...
	function jumpQuanjing(bqId) {
		...
		window.location.href = "${contextpath}/quanjing.htm?currentId=" + bqId
	...
	function getMore(){
		...
		$.ajax({
			type: "post", 
			url: "${contextpath}/quanjingGetMore.htm?secondMenuId="+$("#secondMenuId").val()+"&pageIndex="+pageIndex, 
			dataType: "json",
			success: function (data) { 
				 ...
				 if(jsonArray.list.length==0){
					 ...
					 var strLi='<li class="mui-table-view-cell mui-media"'+
							...
							if($("#secondMenuId").val()=='1086'){
								strLi=strLi+	item.title+''+'<p class="mui-ellipsis-2">'+item.content+'</p>';
							}else{
								strLi=strLi+	item.title+''+'<p class="mui-ellipsis-2">'+item.abstract_content+'</p>';
							}
							strLi=strLi+'</p></div></a></li>';
					 strLis=strLis+strLi;
					 }
