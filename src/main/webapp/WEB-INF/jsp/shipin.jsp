...
<body>
	<header class="mui-bar mui-bar-nav">
		...
		<h1 class="mui-title">${shipin.name }</h1>
		<a id="menu"
		...
		<div class="mui-content-padded">
			...
			<c:forEach var="menu" items="${secondMenuList }">
				<div
					...
					onclick="jumpShipin('${menu.id}')">${menu.name }</div>
			</c:forEach>
		</div>
		<ul class="mui-table-view mui-grid-view">
			<c:forEach var="item" items="${shipinList }">
...
<script>
	...
	jQuery(document).ready(function() {
		$("#header").load("${contextpath}/header.htm?type=shipin");
	...
	function jumpActivityDetail(id){
		window.location.href = '${contextpath}/shipinDetail.htm?id='
			+ id;
	}
	...
	function getMore(){
		...
		$.ajax({
			type: "post", 
			url: "${contextpath}/shipinGetMore.htm?secondMenuId="+$("#secondMenuId").val()+"&pageIndex="+pageIndex, 
			dataType: "json",
